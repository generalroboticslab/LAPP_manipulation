...
class ShadowHandSwingCupGPT(VecTask):
    ...
    def compute_reward(self, actions):
        # self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.left_hand_pos, self.right_hand_pos, self.goal_pos)
        self.rew_buf[:], self.rew_dict = my_compute_reward2(self.object_pos, 
                                                           self.object_rot, 
                                                           self.object_linvel,
    ...
    def my_get_euler_xyz(self, q):
        ...
        pitch = torch.where(
            torch.abs(sinp) >= 1, self.copysign(np.pi / 2.0, sinp), torch.asin(sinp))
    ...
    def xw_init_buf(self):
        self.object_linvel = self.root_state_tensor[self.object_indices, 7:10]
        self.object_rot3 = torch.zeros((2048,3))
        self.left_dist = torch.zeros(2048)
        self.right_dist = torch.zeros(2048)
    ...
    def compute_observations(self):
        ...
        # region [xw] new util
        self.goal_rot3 = self.my_get_euler_xyz(self.goal_rot)
        self.object_rot3 = self.my_get_euler_xyz(self.object_rot)
        
        self.left_dist = torch.norm(self.left_hand_pos - self.cup_left_handle_pos, dim=-1)
        self.right_dist = torch.norm(self.right_hand_pos - self.cup_right_handle_pos, dim=-1)
        # endreigon
        ...
        # region [xw] debug
        rbs_nan_mask = torch.isnan(self.rigid_body_states) | torch.isinf(self.rigid_body_states)
        rbs_exploded_mask_2d = rbs_nan_mask.any(dim=2)  
        rbs_exploded_mask = rbs_exploded_mask_2d.any(dim=1)
        exploded_env_ids = rbs_exploded_mask.nonzero(as_tuple=False).squeeze(-1)
        if len(exploded_env_ids) > 0:
            print(f"Exploded env_ids: {exploded_env_ids}")
            self.reset_buf[exploded_env_ids] = 1

            all_env_ids = torch.arange(self.num_envs, device=self.device)
            non_exploded_mask = ~rbs_exploded_mask
            non_exploded_env_ids = all_env_ids[non_exploded_mask]  # [num_nonexploded]

            if non_exploded_env_ids.numel() == 0:
                print("All envs exploded?? Then we can't replace from others!")
            else:
                random_idxes = torch.randint(
                    low=0,
                    high=non_exploded_env_ids.numel(),
                    size=(exploded_env_ids.numel(),),
                    device=self.device
                )
                replaced_ids = non_exploded_env_ids[random_idxes]  # [K]

                for i in range(exploded_env_ids.numel()):
                    env_bad = exploded_env_ids[i].item()
                    env_good = replaced_ids[i].item()
                    self.obs_buf[env_bad] = self.obs_buf[env_good]
                    self.rew_buf[env_bad] = self.rew_buf[env_good]
                    self.progress_buf[env_bad] = self.progress_buf[env_good]
                    self.successes[env_bad] = self.successes[env_good]
        # endregion
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        if torch.isnan(self.obs_buf).any() or torch.isinf(self.obs_buf).any():
            ...
            inf_indices = torch.nonzero(torch.isinf(self.obs_buf), as_tuple=False)
        
            print(f"NaN/Inf detected:", end=' ')
            if nan_indices.numel() > 0:
                print(f"NaN found at indices: {nan_indices.tolist()}")
            if inf_indices.numel() > 0:
                print(f"Inf found at indices: {inf_indices.tolist()}")
        # endregion
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_relative_control:
            ...
            # region [xw] trial
            # raw_forces = self.actions[:, 0:3] * self.dt * self.transition_scale * 100000
            # raw_torques = self.actions[:, 3:6] * self.dt * self.orientation_scale * 1000
            # raw_forces2 = self.actions[:, 26:29] * self.dt * self.transition_scale * 100000
            # raw_torques2 = self.actions[:, 29:32] * self.dt * self.orientation_scale * 1000 
            # F_max = 500.0
            # T_max = 200.0
            # clamped_forces = torch.clamp(raw_forces, min=-F_max, max=F_max)
            # clamped_torques = torch.clamp(raw_torques, min=-T_max, max=T_max)
            # clamped_forces2 = torch.clamp(raw_forces2, min=-F_max, max=F_max)
            # clamped_torques2 = torch.clamp(raw_torques2, min=-T_max, max=T_max)

            # self.apply_forces[:, 1, :] = clamped_forces
            # self.apply_torque[:, 1, :] = clamped_torques
            # self.apply_forces[:, 1 + 26, :] = clamped_forces2
            # self.apply_torque[:, 1 + 26, :] = clamped_torques2
...
@torch.jit.script
def my_compute_reward(object_pos: torch.Tensor, 
...
@torch.jit.script
def my_compute_reward2(object_pos: torch.Tensor, 
                   object_rot: torch.Tensor, 
                   object_linvel: torch.Tensor, 
                   cup_right_handle_pos: torch.Tensor, 
                   cup_left_handle_pos: torch.Tensor, 
