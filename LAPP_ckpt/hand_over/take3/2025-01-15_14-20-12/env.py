...
class ShadowHandOverGPT(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.palm_another_handle = self.gym.find_asset_rigid_body_index(shadow_hand_asset, 'robot1:palm')

        self.wrist_handle = self.gym.find_asset_rigid_body_index(shadow_hand_asset, 'robot0:wrist')
        self.wrist_another_handle = self.gym.find_asset_rigid_body_index(shadow_hand_asset, 'robot1:wrist')
        # endregion
        ...
        # region new handle
        self.palm_handle = to_torch(self.palm_handle, dtype=torch.long, device=self.device)
        self.palm_another_handle = to_torch(self.palm_another_handle, dtype=torch.long, device=self.device)
        self.wrist_handle = to_torch(self.wrist_handle, dtype=torch.long, device=self.device)
        self.wrist_another_handle = to_torch(self.wrist_another_handle, dtype=torch.long, device=self.device)
        # endregion
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward_v5(self.object_pos, 
                                                           self.object_rot,
                                                           self.object_linvel,
                                                           self.goal_pos, 
                                                           self.goal_rot,
                                                           self.fingertip_pos, 
                                                           self.fingertip_another_pos,
                                                           )
        # region to delete
        # XW: it is same for every env, just take goal_pos[0] is enough
        # print("@@@ Goal pos is:")
        # print(self.goal_pos)
        # endregion
    ...
    def xw_init_buf(self):
        ...
        # self.palm_pos = self.rigid_body_states[:, self.palm_handle][:, 0:3]
        # self.palm_another_pos = self.rigid_body_states[:, self.palm_another_handle][:, 0:3]

        # self.wrist_rot = self.rigid_body_states[:, self.wrist_handle][:, 3:7]
        # self.wrist_rot = self.get_euler_xyz(self.wrist_rot)
        # self.wrist_another_rot = self.rigid_body_states[:, self.wrist_another_handle][:, 3:7]
        # self.wrist_another_rot = self. get_euler_xyz(self.wrist_another_rot)
        
        # self.wrist_angvel = self.rigid_body_states[:, self.wrist_handle][:, 10:13]
        # self.wrist_another_angvel = self.rigid_body_states[:, self.wrist_another_handle][:, 10:13]
        self.dist_to_tip = torch.zeros((2048, 5))
        self.dist_to_another_tip = torch.zeros((2048, 5))
    # endregion
    ...
    def compute_observations(self):
        ...
        # region [xw] new util
        pos_expanded = self.object_pos.unsqueeze(1)
        self.dist_to_tip = torch.norm(pos_expanded - self.fingertip_pos, dim=2)
        self.dist_to_another_tip = torch.norm(pos_expanded - self.fingertip_another_pos, dim=2)
        # endregion
...
def compute_reward_v5(
    object_pos: torch.Tensor, object_rot: torch.Tensor, object_linvel: torch.Tensor,
    ...
    # Calculate the catch reward based on the difference between the object's linear velocity and the goal position
    ...
    for i in range(fingertip_pos.shape[1]):
        ...
        penalty = torch.where(
            (dist_to_fingertip < distance_threshold) | (dist_to_fingertip_another < distance_threshold),
            penalty + 1.0, penalty
        )
