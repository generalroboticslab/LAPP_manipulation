...
class ShadowHandOverGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = LAPP_compute_reward(self.object_pos, 
                                                           self.object_linvel,
                                                           self.goal_pos, 
                                                           self.fingertip_pos, 
    ...
    def _create_recording_camera(self, env_idx=0, width=1280, height=720):
        from isaacgym import gymapi
        camera_props = gymapi.CameraProperties()
        camera_props.width = width
        camera_props.height = height
        # print("$$$ rl device", self.rl_device)
        # print("$$$ graphic id", self.graphics_device_id)

        # if self.envs is None:
        #     print("$$$ envs are None")
        # if self.envs[env_idx] is None:
        #     print("$$$ envs[env_idx] is None")
        camera_handle = self.gym.create_camera_sensor(self.envs[env_idx], camera_props)
        # print("$$$ Find handle")
        # if camera_handle == -1:
        #     print("$$$ Camera handle fail to init!")
        
        cam_pos = gymapi.Vec3(-0.5, -0.5, 0.8)
        cam_target = gymapi.Vec3(0.0, -0.5, 0.5)
        self.gym.set_camera_location(camera_handle, self.envs[env_idx], cam_pos, cam_target)
...
@torch.jit.script
def LAPP_compute_reward(
        object_pos: torch.Tensor, object_linvel: torch.Tensor,
        goal_pos: torch.Tensor, fingertip_pos: torch.Tensor, 
